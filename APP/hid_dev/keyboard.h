...
enum keyboard_sp_key { LEFT_CTRL = 0, LEFT_SHIFT, LEFT_ALT, LEFT_GUI, RIGHT_CTRL, RIGHT_SHIFT, RIGHT_ALT, RIGHT_GUI };
typedef struct
{
...
{
    uint8_t  send_ok;
    uint8_t  mode;           // 0 6keys 1 all keys
    uint8_t  kbd_is_onereport;
    uint8_t  report_length;  //
    uint8_t *report_pointer; //
    uint16_t desp_lenth;
} keyboard_state_t;
...
extern const uint8_t board_non_shift[128];
extern const uint8_t board_with_shift[128];
extern const uint8_t board_keypad[128];
 ...
 */
void keyboard_delay(int8_t time_ms);
/**
 * @brief : 按下键盘上的字符, 键盘ASCII全支持, 还支持 制表符 换行符,
 * 会自动按SHIFT
