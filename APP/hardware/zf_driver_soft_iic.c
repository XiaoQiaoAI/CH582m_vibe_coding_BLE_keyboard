...
#define soft_iic_gpio_high_scl() LL_GPIO_SetOutputPin(soft_iic_obj->iic_scl_gpio_base, soft_iic_obj->scl_pin)  // DC
#define soft_iic_gpio_high_sda() LL_GPIO_SetOutputPin(soft_iic_obj->iic_sda_gpio_base, soft_iic_obj->sda_pin)  // DC
#define soft_iic_gpio_low_scl() LL_GPIO_ResetOutputPin(soft_iic_obj->iic_scl_gpio_base, soft_iic_obj->scl_pin) // DC
#define soft_iic_gpio_low_sda() LL_GPIO_ResetOutputPin(soft_iic_obj->iic_sda_gpio_base, soft_iic_obj->sda_pin) // DC
#define gpio_get_level(x) (LL_GPIO_IsInputPinSet(soft_iic_obj->iic_sda_gpio_base, soft_iic_obj->sda_pin))
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
// #define soft_iic_delay(x) for (__IO uint32 i = x; i--;)
#define soft_iic_delay(x)
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
#define soft_iic_gpio_high(x) soft_iic_gpio_high_scl()
...
static void soft_iic_send_ack(soft_iic_info_struct *soft_iic_obj, uint8 ack)
{
    ...
    if (ack)
    {
        ...
        GPIO_SINGLE_INIT(soft_iic_obj->iic_sda_gpio_base, soft_iic_obj->sda_pin, GPIO_Mode_IPU);
        // MAP_GPIOPinTypeGPIOInput((uint32_t)(soft_iic_obj->iic_sda_gpio_base), (uint8_t)(soft_iic_obj->sda_pin));
#endif
...
#if SOFT_IIC_SDA_IO_SWITCH
        // gpio_set_dir(soft_iic_obj->sda_pin, GPI, GPI_FLOATING_IN);
        GPIO_SINGLE_INIT(soft_iic_obj->iic_sda_gpio_base, soft_iic_obj->sda_pin, GPIO_Mode_IPD);
        // MAP_GPIOPinTypeGPIOInput((uint32_t)(soft_iic_obj->iic_sda_gpio_base), (uint8_t)(soft_iic_obj->sda_pin));
#endif
    ...
    GPIO_SINGLE_INIT(soft_iic_obj->iic_sda_gpio_base, soft_iic_obj->sda_pin, GPIO_Mode_Out_PP);
...
#if SOFT_IIC_SDA_IO_SWITCH
    // gpio_set_dir(soft_iic_obj->sda_pin, GPO, GPO_OPEN_DTAIN);
    // GPIOPinTypeGPIOOutput((uint32_t)(soft_iic_obj->iic_sda_gpio_base), (uint8_t)(soft_iic_obj->sda_pin));
    GPIO_SINGLE_INIT(soft_iic_obj->iic_sda_gpio_base, soft_iic_obj->sda_pin, GPIO_Mode_Out_PP);
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void soft_iic_write_8bit_array(soft_iic_info_struct *soft_iic_obj, uint8 *data, uint32 len)
{
    soft_iic_start(soft_iic_obj);
    soft_iic_send_data(soft_iic_obj, soft_iic_obj->addr << 1);
    while (len--)
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void soft_iic_transfer_8bit_array(soft_iic_info_struct *soft_iic_obj, uint8 *write_data, uint32 write_len, uint8 *read_data, uint32 read_len)
{
    soft_iic_start(soft_iic_obj);
    soft_iic_send_data(soft_iic_obj, soft_iic_obj->addr << 1);
    while (write_len--)
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void soft_iic_transfer_16bit_array(soft_iic_info_struct *soft_iic_obj, uint16 *write_data, uint32 write_len, uint16 *read_data, uint32 read_len)
{
    soft_iic_start(soft_iic_obj);
    soft_iic_send_data(soft_iic_obj, soft_iic_obj->addr << 1);
    while (write_len--)
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void soft_iic_init(soft_iic_info_struct *soft_iic_obj, uint8 addr, uint32 delay, char scl_port, uint32_t scl_pin, char sda_port, uint32_t sda_pin)
{
    soft_iic_obj->scl_pin = scl_pin;
    soft_iic_obj->sda_pin = sda_pin;
    soft_iic_obj->addr = addr;
    soft_iic_obj->delay = delay;
    soft_iic_obj->iic_scl_gpio_base = scl_port;
    soft_iic_obj->iic_sda_gpio_base = sda_port;
    // gpio_init(scl_pin, GPO, GPIO_HIGH, GPO_PUSH_PULL);  // 提取对应IO索引 AF功能编码
    GPIO_SINGLE_INIT(soft_iic_obj->iic_scl_gpio_base, soft_iic_obj->scl_pin, GPIO_Mode_Out_PP);
    GPIO_SINGLE_INIT(soft_iic_obj->iic_sda_gpio_base, soft_iic_obj->sda_pin, GPIO_Mode_Out_PP);
