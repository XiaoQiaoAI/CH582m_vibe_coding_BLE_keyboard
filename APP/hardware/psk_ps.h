...
/*------------------DEBUG DEFINE BEGIN------------------------*/
#ifdef _DEBUG_
#define ps(...) ps_putc((char *) USART_TX_BUF_PS, (unsigned short) sprintf((char *) USART_TX_BUF_PS, __VA_ARGS__))
#else
#define ps(...)
#endif
 ...
 * @retval 无
 */
#define __ValueStep(src, step, max) \
    ((src) = (((step) >= 0) ? (((src) + (step)) % (max)) : (((src) + (max) + (step)) % (max))))
/**
...
#define __ExecuteOnce(func)                     \
    do {                                        \
        static uint8_t isInit_##func## = false; \
        if (!isInit_##func##) {                 \
            func, isInit_##func## = true;       \
        }                                       \
    } while (0)

#define MSB(x) \
    (((x) >> 8) & 0xff)                             /* x占2byte（如short）2byte的高地址的1byte \
                                    */
...
#define AREA(x, min, max) ((x) % ((max) - (min)) + (min))
#endif
