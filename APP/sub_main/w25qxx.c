...
void W25QXX_Init(void)
{
    ...
    if (W25QXX_TYPE == W25Q256)                   //SPI FLASH为W25Q256
    {
        ...
        {
            W25QXX_CS_L;                          //选中
            SPI2_WriteByte(W25X_Enable4ByteAddr); //发送进入4字节地址模式指令
            W25QXX_CS_H;                          //取消片选
        }
    }
}
...
uint8_t W25QXX_ReadSR(uint8_t regno)
{
    ...
    W25QXX_CS_L;             //使能器件
    SPI2_WriteByte(command); //发送读取状态寄存器命令
...
void W25QXX_Write_SR(uint8_t regno, uint8_t sr)
{
    ...
    W25QXX_CS_L;             //使能器件
    SPI2_WriteByte(command); //发送写取状态寄存器命令
    SPI2_WriteByte(sr);      //写入一个字节
    W25QXX_CS_H;             //取消片选
}
//W25QXX写使能
//将WEL置位
...
{
    W25QXX_CS_L;                      //使能器件
    SPI2_WriteByte(W25X_WriteEnable); //发送写使能
    W25QXX_CS_H;                      //取消片选
}
//W25QXX写禁止
//将WEL清零
...
{
    W25QXX_CS_L;                       //使能器件
    SPI2_WriteByte(W25X_WriteDisable); //发送写禁止指令
    W25QXX_CS_H;                       //取消片选
}
...
void W25QXX_Erase_Sector(u32 Dst_Addr)
{
    ...
    SPI2_WriteByte((uint8_t) ((Dst_Addr) >> 16)); //发送24bit地址
    SPI2_WriteByte((uint8_t) ((Dst_Addr) >> 8));
    SPI2_WriteByte((uint8_t) Dst_Addr);
    W25QXX_CS_H;        //取消片选
...
{
    W25QXX_CS_L;                    //使能器件
    SPI2_WriteByte(W25X_PowerDown); //发送掉电命令
    W25QXX_CS_H;                    //取消片选
    DelayUs(3);                     //等待TPD
}
//唤醒
