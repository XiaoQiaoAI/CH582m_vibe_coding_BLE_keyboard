...
uint8_t attDeviceName[GAP_DEVICE_NAME_LEN] = "HID Keyboard";
...
void HidEmu_Init()
{
    ...
    // Setup the GAP Peripheral Role Profile
    {
        ...
        GAPRole_SetParameter(GAPROLE_ADVERT_DATA, (advertData__len), advertData__);
        // GAPRole_SetParameter(GAPROLE_SCAN_RSP_DATA, sizeof(scanRspData),
        // scanRspData);
        GAPRole_SetParameter(GAPROLE_SCAN_RSP_DATA, (scanRspData__len), scanRspData__);
    }
...
uint16_t HidEmu_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    if (events & START_PHY_UPDATE_EVT) {
        ...
        PRINT("Send Phy Update %x...\n",
              GAPRole_UpdatePHY(hidEmuConnHandle, 0, GAP_PHY_BIT_LE_2M, GAP_PHY_BIT_LE_2M, GAP_PHY_OPTIONS_NOPRE));
...
static void hidEmuStateCB(gapRole_States_t newState, gapRoleEvent_t *pEvent)
{
    ...
    case GAPROLE_WAITING:
        if (pEvent->gap.opcode == GAP_END_DISCOVERABLE_DONE_EVENT) {
            ...
            tmos_stop_task(hidDevTaskId, SBP_PERIODIC_EVT);
